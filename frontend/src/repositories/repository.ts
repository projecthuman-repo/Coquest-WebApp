...
class Repository<T extends Model> {
    ...
    fetch(inputObj: T, overrideProps?: Partial<T>, additionalParams?: any): Observable<T> {
        if(inputObj.isValid()) {
            ...
            return from(
                graphQLClient.request(this.createMut, {[`${this.typeName.toLowerCase()}Input`]: toOutputFormat(inputObj)})
            ).pipe(
                map((data: any): T => {
                    ...
                    this.obj = this.factory.create(replaceUndefinedWithNulls({_id: data[`createRegenquest${this.typeName}`]["id"], ...inputObj, ...overrideProps}));
    ...
    update(obj: T) {
        ...
        graphQLClient.request(this.updateMut, {
    ...
    static getInstance<T extends Model>(typeName: RepoTypeName, type: new (data?: any) => T): Repository<T> {
