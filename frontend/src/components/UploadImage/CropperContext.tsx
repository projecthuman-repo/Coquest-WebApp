...
const CroppedImageContext = createContext<CroppedImageContextType | undefined>(
	undefined,
);
...
export const useCroppedImage = () => {
	...
	if (!context) {
		throw new Error(
			"useCroppedImage must be used within a CroppedImageProvider",
		);
	}
	return context;
};
...
export const CroppedImageProvider = ({
	children,
}: CroppedImageProviderProps) => {
