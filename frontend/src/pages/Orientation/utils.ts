...
export async function update(
	updatedUserData: Partial<UserRequired> & Partial<UserOptional>,
) {
	try {
...
export const RegistrationPages: RegistrationPage[] = [
	...
	},
	{
		title: "Purpose",
		view: Purpose,
		dataSetter: (motives: string[]) => {
			update({ motives: Array.from(motives) });
		},
	},
	{
		title: "Interests",
		view: Interests,
		dataSetter: (topics: string[]) => {
			update({ topics: Array.from(topics) });
		},
	},
	{
		title: "Relative location",
		view: RelativeLocation,
		dataSetter: (location: Location) => {
