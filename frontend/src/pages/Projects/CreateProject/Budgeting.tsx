...
const Budgeting: React.FC = () => {
	...
	const [budgetExpenses, setBudgetExpenses] = useState(
		project.budgetExpenses,
	);
	const [totalBudgetExpenses, setTotalBudgetExpenses] = useState(
		project.totalBudgetExpenses,
	);
	const [isOpenToBarter, setIsOpenToBarter] = useState<boolean | undefined>(
		project.isOpenToBarter,
	);

	// participation
	const [cost, setCost] = useState<number | null>(project.cost);
	const [spots, setSpots] = useState<number | null>(project.spots);
	...
	const [crowdfundAmount, setCrowdfundAmount] = useState<number | null>(
		project.crowdfunding.amount,
	);
	const [crowdfundDateOfFunding, setCrowdfundDateOfFunding] =
		useState<Date | null>(project.crowdfunding.dateOfFunding);
	const [crowdfundThankyouMessage, setCrowdfundThankyouMessage] =
		useState<string>(project.crowdfunding.thankyouMessage);
	...
	const updateProjectBudgeting = () => {
		...
		let canProceed =
			updatedProject.budgetExpenses.length > 0 &&
			updatedProject.isOpenToBarter !== undefined &&
			updatedProject.cost !== null &&
			updatedProject.crowdfunding.needed !== undefined;
		if (crowdfundingNeeded) {
			canProceed =
				canProceed &&
				updatedProject.crowdfunding.amount !== null &&
				updatedProject.crowdfunding.dateOfFunding !== null;
		}
				...
				{crowdfundingNeeded && (
					...
					>
						<DateComponent
							...
							value={crowdfundDateOfFunding || null}
